...
const RemoveButton = styled.button`
    ...
    margin-right:20px;
    cursor: pointer;
    ${mobile({width: "40px", height: "40px", borderRadius: "50%", border: "none", backgroundColor: "#d6d1d1", color: "black"})}
    /* ${tab({width: "40px", height: "40px", borderRadius: "50%", border: "none", backgroundColor: "#d6d1d1", color: "black"})} */
`;
...
const Cart = () => {
  ...
  const stripePromise = loadStripe('pk_test_51LVzIiSHbelfXOXs2UwOPxn0UZuulmI2mtyUOnivfXahojRw7F5PsI6ngrI7eXke5oJ5yOSRpDPv8gQECEGfl4Jb00Ujnj9dD1');
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ textAlign: "right" }}>
              <button
                disabled={!stripe}
                className="formBtn5"
                style={{ display: "inline-block", margin: "5px", height: "40px", width: "300px", backgroundColor: "#dcdcef", marginTop: "10px", border:"0.5px solid black", cursor: "pointer"}}
              >
                Submit
              </button>
            </div>
    ...
    const cartDelete = (id) => {
        console.log(id);
        axios.delete(`http://127.0.0.1:8000/cart/cartDelete/${id}/`,
        {
            headers: {
                "Authorization": `Bearer `+localStorage.getItem("access_token"),
                "Content-Type": 'application/json'
    ...
    const checkoutAllPrdoducts = () => {
        axios.get("http://127.0.0.1:8000/checkout/test-checkout/", {
            headers: {
                "Authorization": `Bearer `+localStorage.getItem("access_token"),
                "Content-Type": 'application/json'
            }
    ...
    <Container>
        ...
        <Wrapper>
            ...
            <Top>
                ...
                <TopButton onClick = {orders} type="filled">YOUR ORDERS</TopButton>
                <TopButton onClick = {wishlist} type="filled">WISHLIST</TopButton>
                <TopButton type="filled" onClick={createAddress}>CREATE YOUR ADDRESS</TopButton>
            </Top>
                ...
                <Info>
                        {cart.map((item, index) => {
                            ...
                            return(
                                ...
                                <ProductDetail>
                                    <ImageContainer>
                                        <Image src = {item.product.image} onClick={() => cartDetail(item.product.id)}/>
                                    </ImageContainer>
                                    <Details>
                            ...
                            <PriceDetail>
                                    <ProductAmountContainer>
                                        <Remove onClick = {() => {
                                            if (item.count > 1){
                                                cartCountDecrement(item.product.id, item.count, index)
                                            }else{
                                                console.log("number is below 1");
                                            }
                                        }}/>
                                    <ProductAmount>{item.count}</ProductAmount>
                                        <Add onClick = {() => cartCountIncrement(item.product.id, item.count, index)}/>
                                    </ProductAmountContainer>
                                    <ProductPrice>$ {item.count * item.product.price}</ProductPrice>
                                </PriceDetail>
                                <RemoveIcon>
                                    <RemoveButton onClick={() => cartDelete(item.id)}><DeleteForeverIcon/></RemoveButton>
                                </RemoveIcon>
                            </Product>
